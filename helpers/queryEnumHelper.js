const QUERY_STRING = {
  getQuotaByUnit:
    'SELECT * from quota_usage where "date" = $1 and unit_no = $2',
  getLastLKF: `SELECT * 
    FROM form_lkf fl 
    WHERE station = $1 
      AND "date" = (
  ...
  postFormData: `insert into form_data (from_data_id, no_unit, model_unit, owner, date_trx, hm_last, hm_km, qty_last, qty, flow_start, flow_end, jde_operator, name_operator, start, "end", fbr, lkf_id, signature, type, photo, sync_time, created_by)
  ...
  getShiftStationTK: `SELECT
        fl.lkf_id, fl."date", fl.shift, fl.station, fl.fuelman_id, fl."status", fl.time_opening, fl.note
    FROM form_lkf fl
